...
/// Пофазный учёт времени encode() (включается STEALCODE_PHASE_TIMING=1).
mod timing {
    use std::{
        sync::{Mutex, OnceLock},
    ...
    pub(super) fn enabled() -> bool {
        *ENABLED.get_or_init(|| {
            std::env::var_os("STEALCODE_PHASE_TIMING").is_some()
        })
    }

    pub(super) fn tick(name: &'static str) {
        if !enabled() {
            return;
        }
...
impl PreEncode {
    fn load(gguf: &Gguf, channels: usize, _subsampling: usize) -> Result<Self> {
        ...
        // out: [channels * 17, d_model] вЂ” 17 = floor((128+2+1-3)/2)+1 thrice.
...
impl Encoder {
    pub fn load(gguf: &Gguf, cfg: EncoderConfig) -> Result<Self> {
        ...
        if let (Some(dir), Some(q)) = (
            ...
            let mut s = Vec::new();
            q.to_f32(&mut s);
            std::fs::write(dir.join("w_pre_out.bin"), dump_bytes(&s)).ok();
        }
    ...
    pub fn encode(
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_pre.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(mlp) = &mut self.prompt {
            ...
            if let Some(pid) = prompt_id {
                ...
                // h is [prompt_hidden, t_enc] (mlp0.out rows) — the layout
                // forward_t wants; no transpose.
                let mut y = Vec::new();
                mlp.mlp2.forward_t(&mut self.scratch, &h, t_enc, &mut y);
                // y is [d, t] (mlp2.out rows) — transpose to [t, d].
    ...
    pub(crate) fn pre_encode_forward(
        &mut self,
        mel: &[f32],
        t_mel: usize,
        n_mels: usize,
        ...
        let mut pre_tick = |name: &str, t0: &mut std::time::Instant| {
            if st {
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_c0.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_c2.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_c3.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_c5.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_c6.bin"), dump_bytes(&x)).ok();
        }
        ...
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_flat.bin"), dump_bytes(&flat_x)).ok();
        }
        let xt = transpose(&flat_x, t, flat);
        let mut y_t = Vec::new();
        pe.out.forward_t(&mut self.scratch, &xt, t, &mut y_t);
        if let Some(dir) = &dump_dir {
            std::fs::write(dir.join("enc_lin_t.bin"), dump_bytes(&y_t)).ok();
        }
        let xr = transpose_into(&y_t, d, t, &mut x);
        xr
    }
    ...
    fn block_forward(
        ...
        let dmp = |name: &str, v: &[f32]| {
            if dump {
                if let Some(dir) = dump_dir {
                    std::fs::write(dir.join(name), dump_bytes(v)).ok();
                }
            }
        };
        ...
        // scores[q, k, h] = scale * (q_u.k + q_v.p[k-q+T-1]); вне полосы
        // [k_min, k_max) внимания нет (эквивалент -inf маски) — такие пары
        // не считаем вовсе. Полоса = left_context + chunk_size кадров.
        let chunk_size = right + 1;
