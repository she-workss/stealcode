...
impl GpuContext {
    ...
    pub fn pipeline(
        ...
        let pl = self.device.create_pipeline_layout(
    ...
    /// Copy each `(buffer, size)` into one CPU-visible staging buffer
    /// (at consecutive offsets) and map it back with a single blocking
    ...
    pub fn download_many(&self, bufs: &[(wgpu::Buffer, u64)]) -> Vec<Vec<u8>> {
        ...
        for (buf, size) in bufs {
            encoder.copy_buffer_to_buffer(buf, 0, &staging, off, *size);
            off += size;
        }
        self.queue.submit(Some(encoder.finish()));
        // These can only fail on device loss; the device was just used for
        // the submit above, and wait_indefinitely() blocks until done.
        self.device
        ...
        let _ = rx.recv();
        // map_async's callback ran (we polled past it) and reported Ok.
        let data = slice
            .get_mapped_range()
