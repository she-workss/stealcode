...
use super::{
    context::GpuContext,
    kernels::{PackedMeta, f32_bytes, pack_q8},
};
use crate::nemotron::{
    encoder::Encoder,
    weights::{LayerNorm, Lin},
};
...
fn pack_lin(
    ...
    let packed =
        pack_q8(qm.bytes(), qm.rows(), row_len, qm.padded_row(), block_bytes);
    ...
    let bias = lin
        ...
        .as_ref()
        .map(|b| {
            upload_belt(ctx, belt, encoder, "voice/gpu lin bias", &f32_bytes(b))
        })
        .transpose()?;
...
fn upload_belt(
    ...
    let size_aligned = size.div_ceil(wgpu::COPY_BUFFER_ALIGNMENT)
    ...
    });
    {
        let mut view = belt.write_buffer(
            ...
            &buf,
            0,
            wgpu::BufferSize::new(size_aligned)
                .context("zero-sized weight buffer")?,
        );
...
impl GpuModel {
    ...
    pub fn from_encoder(ctx: &Arc<GpuContext>, enc: &Encoder) -> Result<Self> {
        ...
        let mut belt =
            wgpu::util::StagingBelt::new(ctx.device.as_ref().clone(), 8 << 20);
        for b in &enc.blocks {
            let dw = &b.dw;
            if dw.pad_right != 0 {
                return Err(anyhow!(
            ...
            ctx.device
                ...
                .context("voice/gpu model upload poll")?;
            belt.recall();
            blocks.push(block);
        }
        Ok(Self {
