...
use crate::gguf::{Gguf, f16_to_f32};
...
impl Lin {
    pub fn matvec(&self, x: &[f32], y: &mut Vec<f32>) {
        ...
        if let Some(q) = &self.q {
            ...
            Self::f32_matvec(f, self.inp, self.out, x, y, self.bias.as_deref());
        }
    }
    ...
    pub fn matvec_nb(&self, x: &[f32], y: &mut Vec<f32>) {
        ...
        if let Some(q) = &self.q {
            ...
            Self::f32_matvec(f, self.inp, self.out, x, y, None);
        }
    }
...
impl Conv2d {
    ...
    pub fn forward(
            ...
            && self.stride_f == 1
        {
            ...
            // SAFETY: buffers are sized c_out*c_in, c_in*spatial,
            // c_out*spatial (x is [t*f, c], y_t is [c_out, spatial]);
            // all live and disjoint.
            unsafe {
                gemm(
                    c_out,
                    c_in,
...
impl Conv1dDw {
    ...
    pub fn forward(&self, x: &[f32], t: usize, out: &mut Vec<f32>) {
        self.forward_skip(x, t, 0, out);
    }

    /// Like `forward`, but skips the first `skip` output positions (the
    /// causal padding region, whose values are garbage and discarded).
    /// Result is `[t + pad_left + pad_right - kh + 1 - skip, dim]` with
    /// output `ot - skip` corresponding to input position `ot`.
    pub fn forward_skip(
        &self,
        x: &[f32],
        t: usize,
