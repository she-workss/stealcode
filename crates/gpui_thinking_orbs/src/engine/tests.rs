...
use crate::{
    engine::{
        MAX_ICON_D, MAX_MORPH_DOTS, MAX_NODE_N, ModeOpts, base_profile,
        draw_mode, sanitize_mode_opts,
    },
...
fn new_concepts_are_distinct_low_density_modes() {
    ...
    for (state, mode, avatar_dots, inline_dots) in concepts {
        ...
        for (size, expected) in [
            ...
            let resolved = resolve_preset(state, size);
            let frame =
                draw_mode(resolved.mode, size.pixels(), 0.6, &resolved.opts);
...
fn new_concepts_animate_and_keep_their_depth_identity() {
    for &state in
        &[OrbState::Focusing, OrbState::Reasoning, OrbState::Recalling]
    {
...
fn stroke_colours_are_paintable() {
    ...
    for i in 0..400 {
        let frame =
            draw_mode(resolved.mode, 64.0, i as f32 * 0.05, &resolved.opts);
