...
pub(super) fn vnoise(x: f32, y: f32) -> f32 {
    let xi = x.floor();
    let yi = y.floor();
    let mut fx = x - xi;
    let mut fy = y - yi;
...
pub(super) fn with_fib_dirs<R>(n: usize, f: impl FnOnce(&[Vec3]) -> R) -> R {
...
pub(super) fn with_unit_circle<R>(n: usize, f: impl FnOnce(&[Vec2]) -> R) -> R {
...
#[inline]
pub(super) fn angle_delta(a: f32, b: f32) -> f32 {
...
#[inline]
pub(super) fn radius_scale(size: f32, pow: f32) -> f32 {
    (size / 300.0).powf(pow)
}
