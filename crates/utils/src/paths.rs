...
impl<T: AsRef<Path>> PathExt for T {
    ...
    fn local_to_wsl(&self) -> Option<PathBuf> {
        // quite sketchy to convert this back to path at the end, but a lot of
        // functions only accept paths todo: ideally rework them..?
        ...
        for component in self.as_ref().components() {
            match component {
                ...
                Component::RootDir => {}
                Component::CurDir => new_path.push("/."),
                Component::ParentDir => new_path.push("/.."),
...
/// default, so a path like `.STEALCODE/settings.json` resolves to the same
...
#[must_use]
pub fn component_matches_ignore_ascii_case(
    component: &OsStr,
    name: &str,
) -> bool {
    component
...
/// A safe, owned Arc wrapper for a sanitized path.
/// Since safe Rust cannot construct an `Arc` of a dynamically sized type (DST)
/// without allocations, this struct replaces `Arc<SanitizedPath>`.
#[derive(Debug, Clone, Eq, PartialEq, Hash, Ord, PartialOrd)]
pub struct SanitizedArcPath(Arc<Path>);

impl SanitizedArcPath {
    #[must_use]
    pub fn as_sanitized_path(&self) -> &SanitizedPath {
        SanitizedPath::ref_cast(&self.0)
    }
}

impl std::ops::Deref for SanitizedArcPath {
    type Target = SanitizedPath;

    fn deref(&self) -> &Self::Target {
        self.as_sanitized_path()
    }
}
...
impl SanitizedPath {
    ...
    pub fn new_arc<T: AsRef<Path> + ?Sized>(path: &T) -> SanitizedArcPath {
        Self::from_arc(Arc::from(path.as_ref()))
    }

    #[must_use]
    pub fn cast_arc(path: SanitizedArcPath) -> Arc<Path> {
        path.0
    }

    #[must_use]
    pub const fn cast_arc_ref(path: &SanitizedArcPath) -> &Arc<Path> {
        &path.0
    }
...
impl PathStyle {
    ...
    #[inline]
    #[must_use]
    pub const fn primary_separator(&self) -> &'static str {
        match self {
            Self::Posix => "/",
            Self::Windows => "\\",
        }
    ...
    #[must_use]
    pub const fn separators_ch(&self) -> &'static [char] {
        match self {
            Self::Posix => &['/'],
            Self::Windows => &['\\', '/'],
        }
            ...
            || *self == Self::Windows
                && (path_like.starts_with('\\')
                    || path_like
                        ...
                        && path_like[1..].strip_prefix(':').is_some_and(
                            |path| {
    ...
    pub fn join<P: AsRef<Path>>(self, left: P, right: P) -> Option<String> {
    ...
    pub fn join_path(
        ...
        let right_string = right
            .to_str()
            .ok_or_else(|| anyhow!("Path contains invalid UTF-8"))?;
        let joined = self.join(left, right_string).ok_or_else(|| {
            anyhow!("Path must be relative: {}", right.display())
        })?;
    ...
    pub fn normalize(self, path_like: &str) -> String {
        match self {
            Self::Windows => crate::normalize_path(Path::new(path_like))
                .to_string_lossy()
                .into_owned(),
            Self::Posix => {
    ...
    pub fn strip_prefix<'a>(
        ...
        // Match behavior of std::path::Path, which is case-insensitive for
        // drive letters (e.g., "C:" == "c:")
...
impl PathWithPosition {
    ...
    pub fn parse_str(s: &str) -> Self {
        ...
        let Some(maybe_file_name_with_row_col) =
        ...
        // Let's avoid repeated init cost on this. It is subject to thread
        // contention, but so far this code isn't called from multiple
        // hot paths. Getting contention here in the future seems
        // unlikely.
        match SUFFIX_RE
            ...
            .map(|caps| caps.extract())
        {
            ...
            None => {
                ...
                // but in reality there could be `foo/bar.py:22:in` inputs which
                // we want to match too. The regex mentioned is
                // not very extendable with "digit or random string" checks, so
    ...
    pub fn to_string(
        &self,
        path_to_string: &dyn Fn(&PathBuf) -> String,
    ) -> String {
...
impl PathMatcher {
    ...
    pub fn is_match<P: AsRef<RelPath>>(&self, other: P) -> bool {
        ...
        if self.sources.iter().any(|(_, source, _)| {
            other.starts_with(source) || other.ends_with(source)
        }) {
            return true;
        }
        ...
        self.glob.is_match(
            other_path.into_owned() + self.path_style.primary_separator(),
        )
    }
    ...
    pub fn is_match_std_path<P: AsRef<Path>>(&self, other: P) -> bool {
        let other = other.as_ref();
        if self.sources.iter().any(|(_, source, _)| {
            other.starts_with(source.as_std_path())
                || other.ends_with(source.as_std_path())
        }) {
            return true;
        }
...
pub fn compare_rel_paths_by(
    ...
    let needs_final_tiebreak = mode != SortMode::DirectoriesFirst
        && !(std::ptr::eq(path_a, path_b) || path_a == path_b);
...
mod tests {
    ...
    fn compare_rel_paths_mixed_case_insensitive() {
        ...
        paths.sort_by(|&a, &b| {
            compare_rel_paths_by(a, b, SortMode::Mixed, SortOrder::Default)
        });
    ...
    fn compare_rel_paths_files_first_basic() {
        ...
        paths.sort_by(|&a, &b| {
            compare_rel_paths_by(a, b, SortMode::FilesFirst, SortOrder::Default)
        });
    ...
    fn compare_rel_paths_mixed_same_stem_different_extension() {
        // Files with same stem but different extensions should sort by
        // extension
        let mut paths = vec![
    ...
    fn test_natural_sort() {
        ...
        assert_eq!(
            natural_sort("File-02.txt", "File-2.txt"),
            Ordering::Greater
        );
    ...
    fn test_natural_sort_with_numbers() {
        ...
        assert_eq!(
            natural_sort(
                "file999999999999999999999",
                "file999999999999999999998"
            ),
            Ordering::Greater
        );
        // u128 edge cases
        // Numbers near u128::MAX
    ...
    fn test_natural_sort_case_sensitive() {
        ...
        assert_eq!(natural_sort("File1", "file2"), Ordering::Less);
        assert_eq!(natural_sort("file1", "File2"), Ordering::Less);
        // Numerically equal values: the case-insensitive comparison decides
        // first. Case-sensitive comparison only occurs when both are
